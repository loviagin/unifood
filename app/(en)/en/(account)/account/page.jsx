...
const Account = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        console.log("Loading data for user with ID:", userId);
        ...
        console.log("Status of response:", response.status); // 🔍 Log the error code
        ...
        console.log("Status of response:", ordersData.status);
        if (!ordersData.ok) throw new Error("Error loading data");
          ...
          <div className={styles.cardBottom}>
            <div className={styles.qrContainer}>
              ...
              <span className={styles.qrLabel}>
                Show QR-code at the checkout to spend bonuses
              </span>
            </div>
          </div>
        </div>
          ...
          <div className={styles.infoRow}>
            <div className={styles.card}>
              ...
              <div className={`${styles.accordionContent} ${expandedSection === 'earn' ? styles.expanded : ''}`}>
                <p>Pay for orders and get up to 7% bonuses</p>
              </div>
            </div>
            {/* пишем div */}
            <div className={styles.card}>
              ...
              <div className={`${styles.accordionContent} ${expandedSection === 'spend' ? styles.expanded : ''}`}>
                <p>Pay up to 50% of the order cost with bonuses</p>
              </div>
            </div>
          </div>
                ...
                <div key={item._id} className={styles.historyItem}>
                  <div className={styles.historyInfo}>
                    ...
                    <div className={`${styles.historyBonus} ${item.bonuses < 0 ? styles.spent : ''}`}>
                      {item.bonuses > 0 ? `+${item.bonuses} bonuses` : `Spent ${Math.abs(item.bonuses)} bonuses`}
                    </div>
                  </div>
